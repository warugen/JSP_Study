...
<script>
	function chkJoinInfo(){
		...
		if(frmJoin.email.value.indexOf('@')==-1 ||
				frmJoin.email.value.indexOf('@')=0 ||
				frmJoin.email.value.indexOf('@')==frmJoin.email.value.length-1){
			alert('메일을 확인하세요');
			frmJoin.email.focus();
			return false;
		} */
		...
		<form action="joinPro.jsp" method="post" name="frmJoin" onsubmit="return chkJoinInfo()">
			...
			<table>
				...
				<tr>
					<th>성별</th>
					<td>
						<input type="radio" name="gender" value="남자" checked="checked" id="m"><label for="m">남자</label>
						<input type="radio" name="gender" value="여자" id="f"><label for="f">여자</label>
					</td>
				</tr>
				<tr>
